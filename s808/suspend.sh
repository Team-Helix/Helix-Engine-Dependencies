		...
		if [ -e $AGL ]; then
			...
			if [ -e $AGL ]; then
				...
				if [ "pnp_available" == "false" ]; then
					...
					echo 85 600000:40 672000:58 787200:87 960000:95 1248000:99 > /sys/devices/system/cpu/cpu0/cpufreq/interactive/target_loads
				else
				...
				echo 40000 460000:0 600000:80000 672000:125000 > /sys/devices/system/cpu/cpu0/cpufreq/interactive/above_hispeed_delay
...
if [ -d /sys/devices/system/cpu/cpu4/cpufreq ]; then
	...
	if grep 'schedutil' $AGB; then
		...
		if [ -e $AGB ]; then
			...
			if [ "pnp_available" == "false" ]; then
				echo 85 633000:48 768000:57 1248000:74 1440000:95 1824000:99 > /sys/devices/system/cpu/cpu4/cpufreq/interactive/target_loads
			else
				echo "PnP detected! Tweaks will be set accordingly"
...
echo "4-5" > /sys/devices/system/cpu/offline
