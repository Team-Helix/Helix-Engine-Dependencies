...
echo 18 > /proc/sys/kernel/sched_nr_migrate
...
if [ -d /sys/devices/system/cpu/cpufreq/policy0 ]; then
	...
	if grep 'pwrutilx' $AGL; then
		if [ -e $AGL ]; then
			...
			echo 10000 > $LGP/pwrutilx/up_rate_limit_us
			echo 1000 > $LGP/pwrutilx/down_rate_limit_us
		...
		if [ -e $AGL ]; then
			...
			echo 10000 > $LGP/schedutil/up_rate_limit_us
			echo 500 > $LGP/schedutil/down_rate_limit_us
		...
		if [ -e $AGL ]; then
			...
			echo 90 1401600:95 > $LGP/interactive/target_loads
...
if [ -d /sys/devices/system/cpu/cpufreq/policy4 ]; then
	...
	if grep 'pwrutilx' $AGB; then
		if [ -e $AGB ]; then
			...
			echo 10000 > $BGP/pwrutilx/up_rate_limit_us
		...
		if [ -e $AGB ]; then
			...
			echo 20000 > $BGP/schedutil/up_rate_limit_us
		...
		if [ -e $AGB ]; then
			...
			echo 90 1132800:95 > $BGP/interactive/target_loads
			...
			echo 60000 1132800:100000 > $BGP/interactive/above_hispeed_delay
...
if grep 'schedutil' $AGL; then
	...
	echo "4-7" > /sys/devices/system/cpu/offline
	...
	echo "" > /sys/devices/system/cpu/cpufreq/policy4/affected_cpus
