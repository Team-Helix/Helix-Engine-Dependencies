...
if [ -d /sys/devices/system/cpu/cpu0/cpufreq ]; then
	...
	if grep 'schedutil' $AGL; then
		...
		if [ -e $AGL ]; then
			...
			echo 55 > /proc/sys/kernel/sched_group_downmigrate
			...
			if [ -e /proc/sys/kernel/sched_upmigrate_min_nice ]; then
				echo 3 > /proc/sys/kernel/sched_upmigrate_min_nice
			fi
			...
			echo 33 > /proc/sys/kernel/sched_rr_timeslice_ms
			if [ -e "/proc/sys/kernel/sched_enable_power_aware" ]; then
				echo 1 > /proc/sys/kernel/sched_enable_power_aware
			fi
			...
			if [ -e $AGL ]; then
				...
				if [ "pnp_available" == "false" ]; then
					...
					echo 70 960000:80 1228800:90 1593600:95 > /sys/devices/system/cpu/cpu0/cpufreq/interactive/target_loads
				else
				...
				echo 60000 > $LGP/interactive/min_sample_time	
				echo 99000 > $LGP/interactive/max_freq_hysteresis
...
if [ -d /sys/devices/system/cpu/cpu2/cpufreq ]; then
	...
	if grep 'schedutil' $AGB; then
		...
		if [ -e $AGB ]; then
			...
			if [ "pnp_available" == "false" ]; then
				echo 70 940800:80 1113600:85 1785600:90 2150400:99 > /sys/devices/system/cpu/cpu2/cpufreq/interactive/target_loads
			else
				echo "PnP detected! Tweaks will be set accordingly"
			...
			echo 80000 > $BGP/interactive/timer_slack
			echo $big_max_value > $BGP/interactive/hispeed_freq
			chmod 644 $BGP/interactive/timer_rate
			echo 20000 > $BGP/interactive/timer_rate
			echo 40000 > $BGP/interactive/above_hispeed_delay
			echo 80 > $BGP/interactive/go_hispeed_load
			echo 60000 > $BGP/interactive/min_sample_time		
			echo 99000 > $BGP/interactive/max_freq_hysteresis
...
if [ -e "/sys/module/cpu_boost" ]; then
	...
	echo 0:960000 1:0 2:806400 3:0 > /sys/module/cpu_boost/parameters/input_boost_freq
...
echo 160 > /sys/block/mmcblk0/queue/nr_requests
...
if [ -e /sys/block/zram0 ]; then
	echo 80 > /sys/block/zram0/queue/nr_requests
fi
## GPU
